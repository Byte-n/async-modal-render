...
describe('renderQuiet', () => {
  ...
  it('renderFactoryQuiet should create a quiet renderer', async () => {
    const onSuccess = vi.fn()
    const onError = vi.fn()

    const App = () => {
      const { renderFactoryQuiet, holder } = useAsyncModalRender()
      const openModal = renderFactoryQuiet(TestModal, { title: 'Factory Quiet' })
