...
| renderFactoryQuiet | `AsyncModalRenderQuietFactory` | 安静模式渲染工厂函数 |
| renderFactoryPersistent | `AsyncModalRenderPersistentFactory` | 持久化渲染工厂函数 |
...
包含 `render`, `renderQuiet`, `renderPersistent`, `renderFactory`, `renderFactoryQuiet`, `renderFactoryPersistent`, `destroy` 方法。
不包含 `holder`。
